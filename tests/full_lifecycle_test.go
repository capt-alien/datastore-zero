...
func TestFullLifecycle(t *testing.T) {
	...
	r.Put("/store/{key}", handlers.PutHandler(db))
	r.Get("/store/{key}", handlers.GetHandler(db))
	r.Delete("/store/{key}", handlers.DeleteHandler(db))

	// Step 1: PUT
	putReq := httptest.NewRequest(http.MethodPut, "/store/e2ekey", bytes.NewBufferString("e2evalue"))
	putResp := httptest.NewRecorder()
	r.ServeHTTP(putResp, putReq)
	assert.Equal(t, http.StatusCreated, putResp.Code, "PUT should return 201 Created")

	// Step 2: GET
	getReq := httptest.NewRequest(http.MethodGet, "/store/e2ekey", nil)
	...
	// Step 3: DELETE
	deleteReq := httptest.NewRequest(http.MethodDelete, "/store/e2ekey", nil)
	...
	getReqAgain := httptest.NewRequest(http.MethodGet, "/store/e2ekey", nil)
	...
	assert.Contains(t, getRespAgain.Body.String(), "key not found", "Expected error message after deletion")
}
